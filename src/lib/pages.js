...
export async function getAllPages(options = {}) {
  ...
  // make a loop for each page and run extractClasses on each

  const classes = [];

  pages.forEach((page) => {
    // if page.ctBuilderJson is undefined, skip it
    if (!page.ctBuilderJson) return;
    // else run extractClasses on it
    const classi = extractClasses(JSON.parse(page.ctBuilderJson));

    // add the classes to the classes array
    classes.push(...classi);
 ...
 function extractClasses(node) {
  ...
  if (node.options && Array.isArray(node.options.classes)) {
     classes = node.options.classes;
  }
 
  if (Array.isArray(node.children)) {
     for (let child of node.children) {
       classes = classes.concat(extractClasses(child));
     }
  }
